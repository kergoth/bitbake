...
def try_mirror_url(fetch, origud, ud, ld, check = False):
    ...
    try:
        ...
        mirrortarballs = []
        if hasattr(origud, "mirrortarballs"):
            mirrortarballs.extend(os.path.basename(m) for m in origud.mirrortarballs)
        elif origud.mirrortarball:
            mirrortarballs.append(os.path.basename(origud.mirrortarball))

        if mirrortarballs and os.path.basename(ud.localpath) in mirrortarballs and os.path.basename(ud.localpath) != os.path.basename(origud.localpath):
